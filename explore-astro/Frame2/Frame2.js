...
document.addEventListener('DOMContentLoaded', function () {
  ...
  activeLine.style.width = `${Math.max(firstLogoRect.width, 100)}px`;
  ...
  logos.forEach((logo) => {
      logo.addEventListener('click', () => {
          ...
          activeLine.style.width = `${Math.max(width, 100)}px`; // Мінімум 100 пікселів
      });
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  inputField.addEventListener('input', function() {
      ...
      // Автоматично додаємо точки після днів і місяців
      if (value.length === 2 || value.length === 5) {
          inputField.value += '.';
      }

      // Обмеження введення на формат дд.мм.рррр
      if (value.length > 10) {
