...
document.addEventListener('DOMContentLoaded', function () {
  ...
  activeLine.style.width = `${Math.max(firstLogoRect.width, 220)}px`;
  ...
  logos.forEach((logo) => {
      logo.addEventListener('click', () => {
          ...
          activeLine.style.width = `${Math.max(width, 220)}px`; // Мінімум 100 пікселів
      });
  });
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  inputField.addEventListener('input', function() {
      ...
      // Автоматично додаємо двокрапку після годин
      if (value.length === 2) {
          inputField.value += ':';
      }

      // Обмеження введення на формат гг:хх
      if (value.length > 5) {
